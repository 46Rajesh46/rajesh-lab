...
// Merge physical lines while a "double-quoted" string is still open, so
// multi-line HTML in `show` works. ponytail: escaped `\"` across lines is an edge — add if hit.
function toLogicalLines(src) {
  const out = [];
  let buf = null;
  for (const raw of src.split(/\r?\n/)) {
    const line = buf === null ? raw : buf + '\\n' + raw;
    const quotes = (line.match(/(?<!\\)"/g) || []).length;
    if (quotes % 2 === 1) buf = line;
...
function compile(src) {
  ...
  out += 'http.createServer((req, res) => {\n' +
    ...
    "  if(!entry){ res.statusCode = 404; res.setHeader('Content-Type','text/html'); res.end('404 — no route'); return; }\n" +
    '  const fn = entry[req.method];\n' +
    "  if(!fn){ res.statusCode = 405; res.end('405 — method not allowed'); return; }\n" +
    '  let chunks = [], size = 0, tooBig = false;\n' +
    "  req.on('data', c => { size += c.length; if(size > 1e6){ tooBig = true; req.destroy(); } else chunks.push(c); });\n" +  // 1MB cap — trust boundary
    "  req.on('end', () => {\n" +
    "    if(tooBig){ res.statusCode = 413; res.end('413 — body too large'); return; }\n" +
    ...
    "    } catch(e){ res.statusCode = 400; res.end('400 — bad request body'); return; }\n" +
    '    try { fn(req, res, body); }\n' +
    "    catch(e){ res.statusCode = 500; res.end('500 — ' + e.message); }\n" +
    '  });\n' +
    '}).listen(' + port + ', () => console.log("RealScript ▸ http://localhost:' + port + '"));\n';
  return out;
}

// --- checks -------------------------------------------------------------
function runSelfTest() {
  const js = compile('serve on 8080\nroute "/" do\n  let who = "world"\n  show "hi " + who\nend\n');
  assert(js.includes('res.end(String("hi " + who))'), 'show emitted');
  assert(js.includes('.listen(8080'), 'port emitted');
...
function main() {
  ...
  if (arg === '--selftest') { runSelfTest(); return; }
  if (!arg) { console.error('usage: node realc.js <file.real> [--build]'); process.exit(1); }

  const js = compile(require('fs').readFileSync(arg, 'utf8'));

  if (process.argv.includes('--build')) {
    require('fs').writeFileSync(arg + '.js', js);
    console.log('built ' + arg + '.js');
  } else {
    // Run in its own scope so the emitted `const fs/path` don't clash with ours.
    new Function('require', '__dirname', js)(require, require('path').dirname(require('path').resolve(arg)));
  }
}
